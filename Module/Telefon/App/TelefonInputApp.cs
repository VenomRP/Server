...
namespace GVRP.Module.Telefon.App
{
    public class TelefonInputApp : SimpleApp
    {
        ...
        public void callUserPhone(Player p_Player, int p_CallingNumber)
        {
            ...
            CallUser l_CallerData;

            CallUser l_CallingData;
            ...
            if (LeitstellenPhoneModule.TeamNumberPhones.ContainsKey(p_CallingNumber))
            {
                ...
                var name = selfnumber.ToString() + " (Leitstelle)";

                if (l_Caller.PhoneContacts.TryGetPhoneContactNameByNumber((uint)l_CalledNumber) != null)
                {
                    name = l_Caller.PhoneContacts.TryGetPhoneContactNameByNumber((uint)l_CalledNumber) + " (Leitstelle)";
                }

                l_CallerData = new CallUser()
                {
                    method = "outgoing",
                ...
                l_CallingData = new CallUser()
                {
                    method = "incoming",
                    caller = (int)selfnumber,
                    name = name //l_CalledPlayer.PhoneContacts.TryGetPhoneContactNameByNumber((uint)selfnumber)
                };
