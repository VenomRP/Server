...
namespace GVRP.Module.Robbery
{
    public sealed class RobberyModule : Module<RobberyModule>
    {
        ...
        //automatenmodul @Kayano
        //automatenmodul @Kayano
        public static void automatenraub(DbPlayer iPlayer)
        {
            ...
            var Automat = BankModule.Instance.GetThisAutomat(iPlayer.Player.Position, 15.0f);
             if (Automat != null && !iPlayer.isInjured() && (Math.Abs(iPlayer.Player.Position.Z - Automat.Position.Z) <= 2f))
             {
                Main.m_AsyncThread.AddToAsyncThread(new Task(async () =>
                {
                    ...
                    iPlayer.PlayAnimation(33, "amb@world_human_welding@male@idle_a", "idle_a");
                    Chats.sendProgressBar(iPlayer, 100000);
                    await Task.Delay(100000);
                    iPlayer.Player.TriggerEvent("freezePlayer", true);
                    iPlayer.StopAnimation();
                    var erhalt = rnd.Next(9723, 17239);
                    iPlayer.Container.AddItem(MarkierteScheineID, erhalt);
                    iPlayer.SendNewNotification($"${erhalt} erbeutet!", title: "Raubüberfall");
                    iPlayer.SendNewNotification("Bankautomat aufgebrochen.", PlayerNotification.NotificationType.SUCCESS);
