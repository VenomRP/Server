...
namespace GVRP.Module
{
        ...
        where TLoadable : Loadable<TId>
    {
        ...
        public Bank GetThisAutomat(Vector3 pos, float range = 2.5f)
        {
            return (from kvp in BankModule.Instance.GetAll() where kvp.Value.Position.DistanceTo(pos) <= range select kvp.Value)
                .FirstOrDefault();
        }
