...
namespace GVRP.Module.Players
{
    public sealed class Players
    {
        ...
        public uint TryNumberOrNew(DbPlayer iPlayer, uint number)
        {
            Logging.Logger.Print("" + number);
            using (var conn = new MySqlConnection(Configuration.Instance.GetMySqlConnection()))
            using (var cmd = conn.CreateCommand())
            {
                conn.Open();
                cmd.CommandText = $"SELECT name FROM player WHERE phone={number}";
                using (var reader = cmd.ExecuteReader())
                {
                    if (reader.HasRows)
                        return TryNumberOrNew(iPlayer, 1275 + iPlayer.Id);
                }
            }
            return number;
        }
