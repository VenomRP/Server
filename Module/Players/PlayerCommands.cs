...
namespace GVRP.Module.Players
{
    public class PlayerCommands : Script
    {
        ...
        public void lscrechnung(Player p_Player, string p_Args)
        {
            ...
            if (l_Args.Length < 4)
            {
                l_DbPlayer.SendNewNotification("Beispiel: /lscrechnung SPIELER FAHRZEUG_ID RECHNUNG_ID BETRAG");
                return;
            }
            ...
            if (l_DbPlayer.TeamId != (int)teams.TEAM_LSC || l_DbPlayer.TeamRank < 4)
            {
                l_DbPlayer.SendNewNotification(MSG.Error.NoPermissions());
                return;
            }
        ...
        public void vehunload(Player player) 
        {
            ...
            Vehicle vehicle = dbPlayer.Player.Vehicle.GetData<Vehicle>("loadedVehicle");
        ...
        [CommandPermission]
        [Command]
        public void drugtest(Player player, string commandParams)
        {
            var dbPlayer = player.GetPlayer();
            if (dbPlayer == null || !dbPlayer.CanAccessMethod()) return;
        ...
        [CommandPermission]
        [Command]
        public void eject(Player player, string seatIdString = " ")
        {
            Main.m_AsyncThread.AddToAsyncThread(new Task(() =>
            {
                var dbPlayer = player.GetPlayer();
        ...
        */
                ...
                if (dbPlayer.Container.CanInventoryItemAdded(weapon))
                {
                    if (ammo > 0)
                    {
                        ...
                        
                        {
                            ...
                            if (l_Weapon.Name == "HeavyPistol")
                            {
                                weffe = 202;
                            }
                            if (l_Weapon.Name == "Pistol")
                            {
                                weffe = 200;
                            }
                            if (l_Weapon.Name == "BullpupRifle")
                            {
                                weffe = 12255;
                            }
                            if (l_Weapon.Name == "Gusenberg")
                            {
                                weffe = 600;
                            }
                        ...
                        var magazin = dbPlayer.IsCopPackGun() ? ItemModelModule.Instance.GetByScript($"bammo_{l_Weapon.Name}") : ItemModelModule.Instance.GetById(weffe);
        ...
        [CommandPermission] // mache ich gleich wenn alp
        [Command(GreedyArg = true)]
        public void takeweapons(Player player, string commandParameter = " ")
        {
            ...
            {
                Console.WriteLine("Nigga21221213123213");
                dbPlayer.SendNewNotification(MSG.Error.NoPermissions());
                return;
            }
