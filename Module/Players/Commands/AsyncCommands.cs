    ...
    public class AsyncCommands : Script
    {
        ...
        public async Task SetBlackMoney(Player player, string commandParams)
        {
            await Task.Run(() =>
            {
                ...
                {
                    iPlayer.SendNewNotification(MSG.Error.NoPermissions());
                    return; // wait  if i do /setblackmoney 1212 its crashes | here u have 2 params, player and money
                    // i think its /setblackmoney plrName Value  //hi wait
                    // try
                }
                ...
                var name = findPlayer != null ? findPlayer.GetName() : command[0]; // it takes 2 parameters not only the value, the player name too
