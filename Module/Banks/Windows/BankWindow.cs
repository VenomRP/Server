                ...
                default:
                    ...
                    if (targetPlayer != null && targetPlayer.IsValid() && targetPlayer != iPlayer)
                    {
                        ...
                        if (iPlayer.TakeBankMoney(amount))
                        {
                            ...
                            Discord.SendMessage($"{iPlayer.Player.Name} hat an den Spieler : ({targetPlayer.Player.Name}) den Betrag : ({amount}) überwiesen!", "BANK-LOG", DiscordHandler.Channels.BANKING);
                    ...
                    if (iPlayer.TryData("bankId", out uint bankId))
                    {
                        ...
                        if (auszahlen > 0 && auszahlen <= iPlayer.bank_money[0])
                        {
                            if (iPlayer.TakeBankMoney(auszahlen))
                            {
                                ...
                                Discord.SendMessage($"{iPlayer.Player.Name} hat an der Bank : ({bank.Name}) den Betrag : ({auszahlen}) abgehoben.!", "BANK-LOG", DiscordHandler.Channels.BANKING);
                        ...
                        if (einzahlen > 0 && einzahlen <= iPlayer.money[0])
                        {
                            if (iPlayer.GiveBankMoney(einzahlen))
                            {
                                ...
                                Discord.SendMessage($"{iPlayer.Player.Name} hat an der Bank : ({bank.Name}) den Betrag : ({einzahlen}) eingezahlt.!", "BANK-LOG", DiscordHandler.Channels.BANKING);
