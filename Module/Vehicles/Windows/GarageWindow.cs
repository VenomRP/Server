...
namespace GVRP.Module.Vehicles.Windows
{
    public class GarageWindow : Window<Func<DbPlayer, Garage, bool>>
    {
        ...
        [RemoteEvent]
        public void requestVehicleList(Player client, uint garageId, string state)
        {
        ...
        [RemoteEvent]
        public void requestVehicle(Player client, string state, uint garageId, uint vehicleId)
        {
            var iPlayer = client.GetPlayer();
            if (!iPlayer.IsValid()) return;
