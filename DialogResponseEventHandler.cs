...
namespace GVRP
{
    public class DialogResponseEventHandler : Script
    {
        ...
        public void DialogResponse(Player player, params object[] args)
        {
            ...
            {
                var dialogid2 = iPlayer.watchDialog;
                var input3 = args[0];
                var input4 = "";
                if (args.Length > 1 && Convert.ToString(args[1]) != "")
                {
                    input4 = Convert.ToString(args[1]);
                }

                if (dialogid2 == Dialogs.menu_login)
                {
                    string response = Convert.ToString(input3);
                    ...
                    if (response != "false")
                    {
                        ...
                        if (!string.IsNullOrEmpty(response) || response == null)
                        {
                            iPlayer.SendNewNotification("Passwort erfolgreich erstellt.", title: "SERVER", notificationType: PlayerNotification.NotificationType.SERVER);
                            //var Sha = HashThis.CreateSHA256Hash(response);
                            //var MDNigger = HashThis.CreateMD5Hash(response);

                            //MySQLHandler.ExecuteAsync($"UPDATE player SET Pass = '{Sha}' WHERE id = '{iPlayer.Id}';");
                            //MySQLHandler.ExecuteAsync($"UPDATE player SET Salt = '{MDNigger}' WHERE id = '{iPlayer.Id}';");
                            MySQLHandler.ExecuteAsync($"UPDATE player SET Pw = '1' WHERE id = '{iPlayer.Id}';");
                            //player.Kick("Du musst einmal Reconnecten!");                            
